      ...
      <nav className={`py-4 ${isHomePage ? 'bg-[#e7fb3c]' : 'bg-white border-b border-gray-200'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          {isHomePage && <div className="hidden md:flex items-center space-x-4">
              <Link to="/dashboard" className="bg-black hover:bg-gray-900 text-white font-medium py-2 px-6 rounded-full group">
        ...
        {isMenuOpen && <div className="md:hidden mt-4 pb-4 px-6">
            <div className="flex flex-col space-y-4">
              {isHomePage ? <>
                  <Link to="/dashboard" className="text-black hover:opacity-80 font-medium">
                    Connect wallet
                  </Link>
                </> : <>
